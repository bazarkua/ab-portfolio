...
const SVGMesh: VFC = () => {
	...
	const {
		paths: [path]
	} = useLoader(SVGLoader, '/assets/svg/compass.svg')
	...
	useFrame(({ clock }) => {
		ref.current!.position.y = 1 * Math.sin(clock.getElapsedTime() * 0.3) + 1.5
...
const Floor: VFC = () => {
	...
	return (
		<mesh position-y={-1} rotation={[-Math.PI / 2, 0, Math.PI / 2]}>
